...
class Envira_Gallery_Shortcode {
	...
	public function shortcode( $atts ) {
		...
		$pre_gallery_html = apply_filters( 'envira_abort_gallery_output', false, $data, $gallery_id );
	...
	*/
	public function generate_gallery_item_markup( $gallery, $data, $item, $id, $i ) {
		...
		if ( $this->get_config( 'lightbox_theme', $data ) == 'base_dark' || $this->get_config( 'lightbox_theme', $data ) == 'base_light' ) {
			$lightbox_caption_limit = apply_filters( 'envira_gallery_output_lightbox_caption_limit', 500, $data, $id, $item, $i );
		...
		// Convert line breaks generated by WordPress
		$lightbox_caption = str_replace( array("<br />","<br>","<br/>"), "&lt;br /&gt;", $lightbox_caption); 
                            ...
                            beforeLoad: function(){
                                ...
                                if ( ! $lightbox_images ) {
									...
									if ( !$this->get_config( 'lightbox_title_caption', $data ) || $this->get_config( 'lightbox_title_caption', $data ) != 'title' ) { ?>
										...
										if ( $(this.element).data('envira-lightbox-caption').length > 0 ) {
											// this.title = $(this.element).data('envira-lightbox-caption');
											var lightbox_caption = $(this.element).data('envira-lightbox-caption');
											this.title = $('<div/>').append( lightbox_caption ).text();
										} else {
											this.title = '';
										}
